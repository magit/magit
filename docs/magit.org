...
- User Option: magit-refs-insert-cherry-commits ::

  Whether to insert cherry commits for the branches and tags.

  In repos with a large number of branches and tags, this can take a
  lot of time. Disabling this option can speed up ~magit-show-refs~ by
