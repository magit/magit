...
- Macro: magit-define-popup name doc [group [mode [option]]] :keyword value...
  ...
  - ~:max-action-columns~
    ...
    The maximum number of actions to display on a single line.  This
    helps arranging actions more sensibly.  If there is not enough
    room to display that many actions on one line, then this is
    ignored.
