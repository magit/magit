...
Invoking such a prefix command displays a popup buffer wihch lists the
...
- Macro: magit-define-popup name doc [group [mode [option]]] :keyword value...
  ...
  buffer.  If it is ommitted or ~nil~ then ~magit-popup-mode~ is used.
...
Such a prefix command are a regular interactive command.  Invoking it
shows the available suffix commands and infix arguments in a popup
buffer.  The user can then change the values of the arguments and
finally invoke one of the suffix arguments.  This causes the popup
buffer to disappear and the command to be invoked interactively with
the buffer that was current before the popup buffer was shown current
again and dynamic bindings for ~magit-current-popup~ and
~magit-current-popup-args~ in affect.

Suffix commands also are regular interactive commands.  Usually these
commands use the value of ~magit-current-popup-args~ or preferably the
popup specific ~SHORTNAME-arguments~ inside their ~interactive~ form, but
it is also possible to use existing commands that are not
"popup-aware" as suffix commands.
