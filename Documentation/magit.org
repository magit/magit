...
#+SUBTITLE: for version 2.90.1 (v2.90.1-89-g89f7c0757+1)
...
#+TEXINFO: @noindent
This manual is for Magit version 2.90.1 (v2.90.1-89-g89f7c0757+1).
...
- Key: t t, magit-tag-create
  ...
  First it prompt for the name of the new tag using the highest
  existing tag as initial input and leaving it to the user to
  increment the desired part of the version string.

  Then it prompt for the message of the new tag.  The proposed tag
  ...
  Then it call "git tag --annotate --sign -m MSG TAG" to create the,
  tag, regardless of whether these arguments are enabled in the popup.
  Finally it shows the refs buffer to let the user quickly review the
  result.

- Key: t k, magit-tag-delete

  This command deletes one or more tags.  If the region marks multiple
  tags (and nothing else), then it offer to delete those.  Otherwise,
  it prompt for a single tag to be deleted, defaulting to the tag at
  point.
...
- Command: magit-tag-release
  ...
  This command creates an opinionated release tag.

  It assumes version tags that match "\\`v?[0-9]\\(\\.[0-9]\\)*\\'".
  It prompts for the name of the new tag using the highest existing
  tag as initial input and call "git tag --annotate --sign -m MSG"
  TAG, regardless of whether these arguments are enabled in the popup.
