...
- Key: c r, magit-commit-reword
  ...
  With a prefix argument change the committer date.
  The option ~magit-commit-rewrite-override-date~ can be used to
...
- Some Git variables automatically have an effect in Magit without
  requiring any explicit support.  Sometimes that is desirable, and,
  in other cases, it breaks Magit.
