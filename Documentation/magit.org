...
restriction as that log buffer.  Otherwise, the file restriction is
determined by the file restriction in the repository's diff buffer, if
it exists, and the value of the option ~magit-use-sticky-arguments~.
...
- Key: M-x magit-revision-toggle-file-filter, magit-revision-toggle-file-filter
  ...
  buffer, allowing you to quickly switch between viewing all the
  changes in the commit and the restricted subset.  It is particularly
