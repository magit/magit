...
#+SUBTITLE: for version 2.11.0 (2.11.0-552-g40361273a+1)
...
#+TEXINFO: @noindent
This manual is for Magit version 2.11.0 (2.11.0-552-g40361273a+1).
...
- User Option: magit-refs-primary-column-width
  ...
  Width of the focus column in `magit-refs-mode' buffers.  The primary
  column is the that contains the name the current row is about.
  ...
  case the actual wide is determined using the length of the names of
  the shown local branches.  (Remote branches and tags are not taken
  into account when calculating to optimal wide.)
...
- User Option: magit-refs-focus-column-width
  ...
  For other references it optionally shows how many commits it is
  ahead of the focused branch and ~<~, or if it isn't ahead then the
  commits it is behind and ~>~, or if it isn't behind either, then a ~=~.
...
"the selected modules", but if no modules are selected, then the act
