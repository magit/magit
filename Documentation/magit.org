...
Magit depends on the ~transient~ and ~with-editor~ libraries which are
...
If you haven't installed ~transient~ and ~with-editor~ from Melpa or
at ~/path/to/magit/../<package>~, then you have to tell ~make~ where
to find them. To do so create the file ~/path/to/magit/config.mk~ with
the following content before running ~make~:
