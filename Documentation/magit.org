...
wished they had switches to Emacs sooner so that they would have
...
users are advised to give Evil, the "Extensible VI Layer for Emacs",
and Spacemacs, an "Emacs starter-kit focused on Evil" a try.
...
will likely also start using features that seemed to daunting in the
past.
...
cloning and pushing and other commands that usually don't fail in
...
Also note that staring with the ~2.1.0~ release Magit requires at least
...
This section describes the most essential features which many
...
Depending on what state your repository is in this buffer contains
sections titled "Staged changes", "Unstaged changes", "Unpulled
commits", "Unpushed commits", and some others.
...
Otherwise if there are no uncommitted changes you have to create some
...
these experiments and had uncommitted changes before starting to try
out Magit and also did not create a snapshot, then don't do this.
...
new buffer uses the default name, and therefor will be reused to show
...
- User Option: magit-save-repository-buffers
  ...
  such buffers.  It this is ~dontask~ then this is done without user
...
- Key: g, magit-refresh
  ...
  from ~magit-mode~ as well as the the corresponding status buffer.
...
- User Option: magit-after-revert-hook
  ...
  well as ~magit-not-reverted-hook~ and if possibly remove some of the
...
- User Option: magit-not-reverted-hook
  ...
  extensions with need to run some function even for such files.
...
the global bindings are not shadowed.  To move to another section use
...
- Function: magit-hunk-set-window-start
  ...
  This hook function ensure that the beginning of the current section
...
- Function: magit-log-maybe-show-commit
  ...
  ~magit-diff-auto-show-p~ calls for it, then the commit in shown in
...
- Variable: magit-section-set-visibility-hook
  ...
  the buffer is being created, then hard coded default is used.
...
is only one of the reason why the same key may do something different,
...
yourself, it will automatically be used.
...
- User Option: magit-section-show-child-count
  ...
  only affects sections which could benefit from this information.
...
the library ~magit-key-mode~.  Some future releases will switch to a
successor yet to be written which will likely be named ~transient~.
...
- Key: C-c C-c, magit-dispatch-popup
  ...
  commands as well as some other commands which are not popup commands
  themselves.
...
Currently only the real ~completing-read~ and Ido are fully supported.
...
to be confirmed.  This includes many very common commands and so this
...
- Key: ! k, magit-run-gitk
  ...
  Without a prefix argument run ~gitk --all~, with a prefix argument
  run gitk without any arguments.
...
of these wrappers instead of the binary would degrade perform
horribly.
...
groups; inspecting existing data, manipulate existing data or adding
new data, and transferring data.  Of course that is a rather crude
distinction which often falls short, but it's more useful than no
...
resulted in a conflict -- additional information is displayed which
helps proceeding with that operation, or to abort it.
...
- User Option: magit-log-section-args
  ...
  option is only a temporary kludge and will be removed again.
...
- Key: l o, magit-log
  ...
  ranges, but only branches and tags, and a representation of the
...
humans cannot remember hashes or "HEAD~5", at least not with double
...
- Key: e, magit-ediff-dwim
  ...
  be able to guess either the file, or range or commit, in which case
  ...
  explicitly.  If it cannot read the users mind at all, then it asks
...
- Key: y o, magit-show-refs
  ...
  List and compare references in a dedicated buffer.  Refs are
  compared with a branch read form the user.
...
Like Git Magit can of course stage and unstage not only complete
files.  Unlike Git it also allows users to gracefully un-/stage
...
modal and rather clumsy interface by using an ~git add --interactive~
session.
...
in the status buffer or a separate diff buffer and type ~s~/~u~.  To
...
"apply variants" which can also operate on a file, multiple files at
...
finishes the editing session the Emacsclient exits Git creates the
...
- Key: c e, magit-commit-extend
  ...
  argument do change the committer date, otherwise don't.  The option
...
- Key: c r, magit-commit-reword
  ...
  don't.  The option ~magit-commit-rewrite-override-date~ can be used to
  inverse the meaning of the prefix argument.
...
The messages are not only used by Git, they may also stored in a ring
which persists until Emacs is closed.  By default the message is
...
The ~git-commit-mode~ is a minor mode which is only used to establish
...
Because ~git-commit-mode~ is a minor mode, we don't use it's mode hook
...
- Function: git-commit-setup-changelog-support
  ...
  After this function was called, ChangeLog entries are treated as
  paragraphs.
...
- Function: git-commit-check-style-conventions
  ...
  sure the summary line isn't to long and that the second line is
  empty.
...
- Key: b c, magit-branch
  ...
  revision used as the starting point of the new branch.  When a
...
- Key: b r, magit-branch-rename
  ...
  Rename branch a branch.  The branch and the new name are read in the
  minibuffer.  With prefix argument the branch is rename even if that
...
When no merge is not in progress, then the popup buffer features the
following commands.
...
- Key: m m, magit-merge
  ...
  does not get a change to inspect or change it before the commit is
...
When no rebase is not in progress, then the popup buffer features the
following commands.
...
- Key: r o, magit-rebase-from
  ...
  Start a non-interactive rebase sequence.  Commits from START to
...
- Key: f, git-rebase-fixup
  ...
  Meld commit on current line into previous commit, discard its
  message.
...
- Key: A a, magit-cherry-apply
  ...
  This command also has a top-level binding, it can be invoked without
...
- Key: M-x magit-reset-soft, magit-reset-soft
  ...
  Reset the ~HEAD~ to to some commit read from the user and defaulting
...
- Key: T s, magit-notes-set-ref
  ...
  ~core.notesRef~.  With a prefix argument change the global value, else
  the value in the current repository.  When this is undefined, then
  "refs/notes/commit" is used.
...
- Key: T S, magit-notes-set-display-refs
  ...
  prefix argument GLOBAL change the global values, else the values in
...
For that reason Magit provides three global modes which save *tracked*
...
There exists a total of three global modes which save to the wip refs,
which might seem excessive, but allows fine tuning when exactly
...
that.  So an additional mode exists which does commit to the wip refs
...
- Certain settings like e.g. ~fetch.prune=true~ are respected by Magit
...
The next two sections list and discuss several variables which many
users might want to customize, for safety and/or performance reasons.

*** Safety

This section discusses various variables which you might want to
...
process.  For that reason Magit provides three global modes which save
...
turn would refuse to carry out the action which would cause these
...
because there might be some other users doing such things, and don't
...
I do care about performance and over the last few months have been
constantly tweaking but many features do come with an inherent
performance penalty.  When a feature is just too slow given certain
usage and repository characteristics then it often can be disabled,
globally or on a per-repository basis.

But first a short list of performance issues that cannot easily be
worked around:

- Creating a new process on MS Windows is much slower than on posix
  compatible systems.  This is a problem because Magit creates a lot
  of child processes to retrieve information from Git.  In the short
  run only switching to a posix system can fix this.  Of course this
  could also be fixed in Windows itself, but I am just one unpaid
...
these functions have one of the prefixes ~magit-run-~, ~magit-call-~,
~magit-start-~, and ~magit-git-~ (which is also used for other things);
and are defined in either ~magit-git.el~ or ~magit-process.el~.
...
- Function: magit-git-str &rest args
  ...
  This is a variant of ~magit-git-string~ which ignores the option
  ...
  therefor lead to an infinite recursion.  You probably won't ever
...
- Function: magit-region-sections
  ...
  uses the ~magit-section-highlight~.  This does not apply to diffs were
...
- Function: magit-diff-type &optional section
  ...
  these values are not handled explicitly everywhere they should be
  and a possible fix could be to just return nil here.
...
- Function: magit-diff-scope &optional section strict
  ...
  i.e. ~hunks~ or ~files~.  If SECTION, or if that is nil the current
  section, is a ~hunk~ section; and the region region starts and ends
  inside the body of a that section, then the type is ~region~.
...
- Function: magit-mode-display-buffer buffer mode &optional switch-function
  ...
  is ~nil~ then ~pop-to-buffer~ is used provided the current buffer's
  major mode derives from ~magit-mode~; else ~switch-to-buffer~ is used.
...
- Variable: magit-refresh-args
  ...
  its also useful to provide commands which can change the value.  For
  ...
  again which differences should be shown, and ~magit-diff-more-context~,
...
Dangerous operations which may lead to data loss, by default have to
be confirmed.  With a multi-section selection this is done using
questions which can be answered with yes or no.  When the region isn't
...
property backgrounds.  That's no longer necessary, highlighting no
...
color which really sticks out, which is ugly but if that were the only
...
some foreground color is to close to the region background color, to
...
~default~.  The highlight background color must be different from the
~region~ background color, not only the ~default~ background color.

For diff related sections Magit uses various different faces to
...
instead use on of the faces ~magit-diff-file-heading-selection~ or
~magit-diff-hunk-heading-selection~.  These faces have to be different
from the regular highlight variants, otherwise there would be no
...
installing the built time dependencies.
...
probably have a reason to have set that variable, and should therefor
