...
- Function: magit-display-buffer-traditional buffer
  ...
  This function is the current default value of option
...
- Function: magit-maybe-set-dedicated
  ...
  This function remembers when a new window was created to display the
  buffer.  That information is used by ~magit-mode-quit-window~, to
  determine whether the window should be deleted when its last Magit
  buffer is buried.
...
- User Option: magit-generate-buffer-name-function
  ...
  Such a function should take ~magit-uniquify-buffer-names~ as well as
  ~magit-buffer-name-format~ into account.  If it doesn't, then its
  doc-string should say so.  If it supports %-sequences not mentioned
  in the doc-string of ~magit-buffer-name-format~, then its own
  doc-string should describe them.
...
- Function: magit-generate-buffer-name-default-function mode
  ...
  This function uses ~magit-buffer-name-format~, supporting the
  %-sequences mentioned the documentation of that option.  It also
  respects ~magit-uniquify-buffer-names~.
...
- User Option: magit-buffer-name-format
  ...
  - ~%v~ The value the buffer is locked to, in parentheses, if it is the
    buffer is actually locked to its value.  An empty string
    otherwise.

  - ~%V~ Like ~%v~, but the string is prefixed with a space, unless it is
    an empty string.

  - ~%t~ The top-level directory of the working tree of the repository,
    or if ~magit-uniquify-buffer-name~ is non-nil an abbreviation of
    that.

  The value should always contain either ~%m~ or ~%M~ as well as ~%t~.  If
  ~magit-uniquify-buffer-name~ is non-nil, then the value must end with
  ~%t~.
...
- User Option: magit-uniquify-buffer-names
  ...
  contain the full name to the top-level of the working tree of the
  ...
  This is done using the ~uniquify~ package; custoize its options to
...
repository (see ~magit-refresh-status~ below).
