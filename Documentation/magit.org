...
The patch has been backported, so you get it not only when using
~--HEAD~, but also when using ~--devel~ or when installing the latest
release (by not using a version argument).  The patch was added only
recently, so you will likely have to reinstall Emacs.

Alternatively you have to apply the patch [fn:mac3] manually.
...
[fn:mac3] https://github.com/emacs-mirror/emacs/commit/a13eaddce2ddbe3ba0b7f4c81715bc0fcdba99f6
