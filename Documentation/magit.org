...
#+SUBTITLE: for version 2.11.0 (2.11.0-484-ge26b79050+1)
...
#+TEXINFO: @noindent
This manual is for Magit version 2.11.0 (2.11.0-484-ge26b79050+1).
...
The following commands not apply some commits to some branch, but also
remove them from some other branch.  The removal is performed using
...
same you would have to if these commands didn't exist at all.

- Key: A h, magit-cherry-harvest

  This command moves the selected COMMITS that must be located on
  another BRANCH onto the current branch instead, removing them the
  former.  When this command succeeds, then the same branch is current
  as before.
