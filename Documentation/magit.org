...
- Function: magit-after-save-refresh-status
  ...
  obviously not make it worse by adding this function to that hook.
...
** Saving of File-Visiting Buffers
...
doubt or after making outside changes, type ~g~ (~magit-refresh~) to save
...
** Reverting of File-Visiting Buffers

Magit can automatically revert buffers that are visiting files that
are being tracked in the current Git repository, after running a Git
command changed the file on disk.  This is useful because it makes it
possible to continue editing a buffer after e.g.  performing a merge,
without having to first answer the question "<file> changed on disk;
really edit the buffer?" and then later also "<file> has changed since
visited or saved.  Save anyway?".

In the past this feature was enabled by default, but beginning with
the v2.4 release, Magit provides two competing implementations which
all user have to choose from.  Neither of the implementations is
enabled by default, but for users who have previously set the option
~magit-revert-buffers~ to a non-nil value, the old implementation
continues to be used.

The new implementation is both faster and simpler but unfortunately it
can only be used if the operating system supports file notifications
and if Emacs supports the file notification implementation of the
operating system.

You should determine whether you want file-visiting buffers to be
automatically reverted at all (see [[*Risk of Reverting Automatically]]),
and if so whether you can use the new implementation.  You should also
consider using the generic implementation, which comes with Emacs.

*** New Auto-Revert Implementation

Magit's new auto-revert implementation is built on top of the generic
implementation, which is part of Emacs itself.  It implements as a
global minor mode named ~magit-auto-revert-mode~.  This mode works by
turning on Emacs' local minor mode ~auto-revert-mode~ in all buffers
which are visiting tracked files located inside any Git repository at
the time a buffer is first created.

By default this global mode only turns on the local mode in buffers
that visit /tracked/ files.  If you create a new file, visit it in a
buffer, and then stage it using a Magit command, then the local mode
is also automatically turned on.  But if you stage the file from
outside Magit, then Magit won't notice that and you have to turn on
the local mode manually.  Magit also doesn't notice if a previously
tracked file is no longer tracked.  In other words, the new
implementation uses a cache.  The old implementation doesn't use a
cache and that is the main reason for it's slowness.

If you set the option ~magit-auto-revert-tracked-only~ to ~nil~, then the
local mode is enabled in all buffer that visit any file inside Git
repositories, including files which are not being tracked by Git and
files inside Git's control directory.

Emacs itself provides a global minor mode (~global-auto-revert-mode~),
which you might want to turn on /instead of/ ~magit-auto-revert-mode~.
That mode is agnostic to Git, it affects all file-visiting buffers,
but if you do like files inside Git repositories to be automatically
reverted, then you might very well like /all/ files to be automatically
reverted.

Unfortunately both ~magit-auto-revert-mode~ and ~global-auto-revert-mode~
are only suitable for use by Magit if it is possible to use file
notifications.  If that is so, then the operating system immediately
informs Emacs when a file changed on disk, and Emacs then immediately
reverts the respective buffers.

If ~auto-revert-use-notify~ is ~nil~, then file notifications are never
used, so make sure the value is ~t~.  Also do not forget to set
~magit-revert-buffers~ to ~nil~ if you use ~magit-auto-revert-mode~ or
~global-auto-revert-mode~.

If the operating system does not support file notifications, or Emacs
does not support that particular implementation, then the reverts do
/not/ happen immediately.  Instead they happen after the user was
inactive for (by default) five seconds.  But we want the reverts to
happen immediately, because otherwise you would have to wait five
seconds after e.g. checking out another branch until you could start
editing an affected buffer.  This would be undesirable because simply
saying /yes/ when asked "<file> changed on disk; really edit the
buffer?"  would be faster than counting to five.  So we might as well
not automatically revert at all.

On GNU/Linux file notifications are expected to be supported.  On OS X
and Windows this likely isn't the case.  If file notifications are not
fully supported, then you unfortunately have to use the old
implementation instead.

- User Option: magit-auto-revert-mode

  When this mode is enabled, then all unmodified buffers that visit
  files that are being tracked by a Git repository are automatically
  reverted as soon as the respective file changes on disk.

- User Option: magit-auto-revert-tracked-only

  When this option is ~nil~, then ~magit-auto-revert-mode~ also reverts
  untracked files and files inside Git's control directory.

*** Old Auto-Revert Implementation

Magit's old auto-revert implementation is too slow because, among
other issues, it does not cache whether a file is being tracked in its
Git repository or not.  Magit now provides a new implementation which
does cache that information, and you should investigate whether you
can use that.

If it turns out that you cannot use the new implementation, then you
should consider setting ~magit-revert-buffers-only-for-tracked-files~ to
~nil~ instead.  Doing that should also improve performace.
...
For most users, automatically reverting file-visiting buffers isn't as
risky as it might sound.

If a buffer is modified (i.e. it contains changes that haven't been
saved yet), then Emacs never automatically reverts it.  If the buffer
is unmodified, resulting in what is seen by Git as an uncommitted
change, then Git in turn would refuse to carry out any commands that
would cause these changes to be lost.  Since Git doesn't do anything,
the file doesn't change on disk, and Emacs has no reason to revert it.

But if you do use file-visiting buffers as a sort of "staging area",
then the automatic reverts could be harmful.  An example: You visit
some file in Emacs, edit it, and save the changes.  Then, outside of
Emacs (or at least not using Magit) you change the file on disk.  At
this point the automatic revert would kick in because the buffer has
no unsaved changes (i.e. the buffer is unmodified).  If your work flow
depends on that not happening, then you cannot use any of the
auto-revert modes, because that's exactly what they do.
...
want to be responsible for data loss, Magit does not turn on any of
the available auto-revert modes by default.

Depending on your work flow, setting ~magit-auto-revert-tracked-only~
resp.  ~magit-revert-buffers-only-for-tracked-files~ to ~nil~ might, or
might not, increase the risk of data loss.
...
By default buffers visiting files tracked in the current repository
are being reverted after certain actions.  This isn't as risky as it
might sound, but you have to be aware that this is happening because
if you are using an unusual work flow, then you might be at risk.  See
[[*Reverting of File-Visiting Buffers]] for an in-depth discussion.
...
- ~magit-revert-buffers~
  ...
  If this is non-nil, then the old auto-revert implementation is used.
  There also exists a new implementation which is much faster.  The
  latter isn't used by default because it is only suitable if file
  notifications are fully supported.  See [[*Reverting of File-Visiting
  Buffers]] to figure out whether you can use the new implementation.

  If you are forced to stick with the old implementation, then also
  consider setting ~magit-revert-buffers-only-for-tracked-files~ to ~nil~,
  and check whether a third-party package has added something slow to
  ~magit-after-revert-hook~ and/or ~magit-not-reverted-hook~.
