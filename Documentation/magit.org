...
- Key: e, magit-ediff-dwim
  ...
  be able to guess either the file, range, or commit, in which case
...
these functions are defined in either ~magit-git.el~ or
~magit-process.el~ and have one of the following prefixes:
~magit-run-~, ~magit-call-~, ~magit-start-~, and ~magit-git-~ (which
is also used for other things).
...
- Function: magit-mode-display-buffer buffer mode &optional switch-function
  ...
  is ~nil~ then ~pop-to-buffer~ is used if the current buffer's
  major mode derives from ~magit-mode~.  ~switch-to-buffer~ is used
  otherwise.
...
instead use one of two faces, ~magit-diff-file-heading-selection~ or
