...
(magit-define-popup magit-log-popup
  ...
  'magit-popups
  :man-page "git-log"
  :switches '((?m "Show only merges"        "--merges")
              (?d "Order by timestamps"     "--date-order")
              (?f "Show only first parent"  "--first-parent")
              (?i "Search ignoring case"    "--regexp-ignore-case")
              (?P "Use extended regexs"     "--pickaxe-regex")
              (?g "Show graph"              "--graph")
              (?u "Show diffs"              "--patch")
              (?s "Show diffstats"          "--stat")
              (?S "Show signatures"         "--show-signature")
              (?D "Show refnames"           "--decorate")
              (?n "List modified files"     "--name-only")
              (?M "All match"               "--all-match")
              (?A "Show all refs"           "--all"))
  :options  '((?r "Limit to subdirectory"   "--relative="  read-directory-name)
              (?c "Limit to committer"      "--committer=" read-from-minibuffer)
              ...
              (?b "Show branches"           "--branches="  read-from-minibuffer)
              (?R "Show remotes"            "--remotes="   read-from-minibuffer))
  :actions  '((?l "Log current"             magit-log-dwim)
              ...
              (?b "Log other"               magit-log)
              (?B "Log other (verbose)"     magit-log-verbose)
