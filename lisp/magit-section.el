    ...
    (let ((inhibit-read-only t)
      ...
      (save-excursion
        (if (and content (< content (oref section end)))
          ...
          (setf (oref section content) (point-marker))
          (funcall washer)
          (setf (oref section end) (point-marker)))))
    ...
    `(let* ((,tp ,(let ((type (nth 0 (car args))))
       ...
       (let ((magit-insert-section--current ,s)
         ...
         (catch 'cancel-section
           ...
           (let* ((end (setf (oref ,s end) (point-marker)))
           ...
           (if (eq ,s magit-root-section)
             ...
             (setf (oref (oref ,s parent) children)
    ...
    (when header-sections
      ...
      (when (cdr header-sections)
        ...
        (let* ((1st-header (pop header-sections))
          ...
          (setf (oref header-parent children) (list 1st-header))
          (setf (oref 1st-header children) header-sections)
          (setf (oref 1st-header content) (oref (car header-sections) start))
          (setf (oref 1st-header end) (oref (car (last header-sections)) end))
          (dolist (sub-header header-sections)
            (setf (oref sub-header parent) 1st-header)))))))
